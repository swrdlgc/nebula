...
public class CompositeTableSnippet7 {
	...
	public static void main (String [] args) {
	    ...
	    shell.setText("CompositeTable Snippet 7 -- Display a table and show hints below");
	    shell.setLayout(new FillLayout());

	    final CompositeTable table = new CompositeTable(shell, SWT.NULL);
	    new Header(table, SWT.NULL); // Just drop the Header and Row in that order...
	    new Row(table, SWT.NULL);
	    table.setRunTime(true);
	    table.setNumRowsInCollection(4000); // some arbitrary number for now
	    table.setMaxRowsVisible(20);
	    table.setInsertHint("Class CompositeTable."
	    		+ "\n(1) An SWT virtual table control that extends Composite. "
	    		+ "\n(2) An SWT virtual table control that is composed of many "
	    ...
	    table.addRowContentProvider(new IRowContentProvider() {
			public void refresh(CompositeTable sender, int currentObjectOffset,
				...
				row.firstName.setText("first" + currentObjectOffset);
				row.lastName.setText("last" + currentObjectOffset);
			}
		});
